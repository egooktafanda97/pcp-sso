...
import { get, destroy } from '@/components/provider/api'

export default function Store({ roles, id }: any) {
    const handlingSave = (state: any) => {
        ;(async () => {
            const axs: any = await axios.post(`/user-management/${id}`, state).catch((err) => {
                Swal.fire({
                    icon: 'error',
                    title: 'Oops...',
    ...
    return (
        <Layouts>
            <ElementState
                ...
                init={({ state, set }) => {
                    if (id)
                        get(`/user-management/usr/${id}`, { alert: false }).then((res) => {
                            set({
                                name: res.name,
                        ...
                        <div className="grid gap-6 mb-6 md:grid-cols-2">
                            ...
                            <div>
                                ...
                                >
                                    ...
                                    {roles &&
                                        roles.map((role: any) => (
                                            <option key={role.id} value={role.id} selected={state.role == role.id ? true : undefined}>
                                                {role.name}
                                            </option>
                                        ))}
                                </select>
